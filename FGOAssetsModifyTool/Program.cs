...
namespace FGOAssetsModifyTool
{
	class Program
	{
		...
		static Dictionary<string, string> ScriptReplaceRules = new();
		static async void DisplayMenuAsync()
		{
			...
			try
			{
				...
				switch (arg)
				{
					...
					case 11:
						{
							ScriptReplaceRules = JsonSerializer.Deserialize<Dictionary<string, string>>(File.ReadAllText(@"ScriptReplaceRules.json"));
							break;
						}
					case 12:
						{
							...
							var CN = JsonSerializer.Deserialize<Dictionary<string, string>>(CNText, options);
							JsonObject NoTranslation = new();
							foreach (var node in JP)
							{
								string val;
								if (CN.TryGetValue(node.Key, out val))
								{
									JP[node.Key] = val;
								}
								else
